...
关键导出：
- NovelSystemService: 主服务类，封装所有业务逻辑
- create_service: 工厂函数，创建配置好的服务实例
...
class NovelSystemService:
    ...
    封装问答、续写、索引构建等核心业务逻辑。典型用法：

        service = NovelSystemService()
        response = service.ask(book_id, AskRequest(user_query="韩立是谁"))

    主要入口：
        ask(): 问答接口
        continue_story(): 续写接口
        index_book() / start_book_index(): 索引构建
    """
    ...
    def ask(self, book_id: str, request: AskRequest) -> AskResponse:
        ...
        执行流程：查询规划 → 查询重写 → 检索 → LLM 生成 → 验证 → 返回结果。

        Args:
            book_id: 书籍 ID
            request: 问答请求，包含 user_query、scope、debug 等字段

        Returns:
            AskResponse: 包含 answer、evidence、confidence 等字段
        """
        # === TRACING: 初始化追踪上下文 ===
    ...
    def continue_story(self, book_id: str, request: ContinueRequest) -> ContinuationResponse:
        ...
        基于当前范围的人物、剧情、风格生成续写内容。

        Args:
            book_id: 书籍 ID
            request: 续写请求，包含 user_query、scope 等字段

        Returns:
            ContinuationResponse: 包续写文本、evidence、validation 结果
        """
        # === TRACING: 初始化追踪上下文 ===
        trace_id = TraceLogger.generate_trace_id()
        start_time = time.perf_counter()
    ...
    def _retrieve_with_rewrite(
        ...
        检索策略：重写查询为主召回，原始查询为补充召回，合并去重后返回。

        Args:
            book_index: 书籍索引对象
            rewritten: 重写后的查询对象，包含 original 和 rewritten 字段
            planner: 查询规划结果
            scope: 章节范围
            top_k: 返回结果数量上限
            simulate: 测试模拟模式
    ...
    def _execute_answer_skill(
        ...
        *,
        ...
        try:
            # 300 tokens 足够：prompt 要求 1-2 句简洁中文答案（约 50-150 字），
            # 即使 analysis 类任务带简短证据也在 300 tokens 内。
