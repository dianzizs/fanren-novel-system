...
class BookIndexRepository:
    ...
    def _extract_structured_from_llm(
        ...
        cache_path = None
        if hasattr(self, "_active_book_id") and self._active_book_id:
            chunk_hash = hashlib.md5(chunk_text.encode("utf-8")).hexdigest()
            cache_dir = self._book_dir(self._active_book_id) / ".llm_cache"
    ...
    def prewarm_llm_extractions(
        ...
        for chapter in chapters:
            self._active_llm_extractions[chapter["chapter"]] = []
        ...
        def worker_task(ch_num: int, chunk_idx: int, chunk_text: str):
            with semaphore:
                try:
                    ext = self._extract_structured_from_llm(chunk_text, token_callback)
        ...
        for ch_num, slots in temp_slots.items():
            self._active_llm_extractions[ch_num] = [x for x in slots if x is not None]
